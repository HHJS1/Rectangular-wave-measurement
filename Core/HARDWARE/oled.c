...
// 将浮点数按小数位数格式化并用 ASCII 字体打印
void OLED_PrintFloat(uint8_t x, uint8_t y, float value, uint8_t decimal, const ASCIIFont *font, OLED_ColorMode color) {
    char buf[32];
    if (decimal > 10) decimal = 10; // 限制精度避免超出缓冲
    snprintf(buf, sizeof(buf), "%.*f", decimal, value);
    OLED_PrintASCIIString(x, y, buf, (ASCIIFont *)font, color);
}

// 将浮点数按小数位数格式化并用通用 Font（支持中文字模）打印
void OLED_PrintFloatWithFont(uint8_t x, uint8_t y, float value, uint8_t decimal, const Font *font, OLED_ColorMode color) {
    char buf[64];
    if (decimal > 10) decimal = 10;
    snprintf(buf, sizeof(buf), "%.*f", decimal, value);
    OLED_PrintString(x, y, buf, (Font *)font, color);
}